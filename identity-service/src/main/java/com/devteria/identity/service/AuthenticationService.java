...
import com.devteria.identity.repository.UserSessionRepository;
...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        if (userSession == null) {
            ...
            if (!userSession.getExpiresAt().after(new Date())){
                var token = generateToken(user);
                userSession.setJwt(token.token);
                userSession.setExpiresAt(token.expiryDate);
                ...
                throw new AppException(ErrorCode.already_logged);
            }
        }
    }
...
//                .toEpochMilli());
        ...
        // Sử dụng Calendar để tính thời gian hết hạn
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(issueTime);
        calendar.add(Calendar.HOUR, 1); // Thêm 1 giờ
